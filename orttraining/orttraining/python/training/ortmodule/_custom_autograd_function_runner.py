...
def call_python_forward_function(
    ...
    def wrap_all_outputs(result, tensors_generated_by_ort: List[torch.Tensor], training_mode_flag: bool):
        ...
        def register_context(result):
            ...
            if training_mode_flag and ctx:
                ...
                # Check tensors generated by ORT is in the saved_tensors or not. If it exists, we throw an exception
                # to let users modify the code by explicitly calling copy before saving the tensor in context.
                # This is because ORTModule will not be able to be aware of the reference count of the tensor in context,
                # which may cause the tensor to be freed by ORT before the backward function is called.
                for tensor in tensors_generated_by_ort:
                    if tensor in saved_tensors:
                        msg = (
                            "Tensor generated by ORT is saved in context, but ORT cannot be aware of the "
                            "additional reference for this tensor. Please call clone() before saving it in context."
                        )
                        warnings.warn(msg)
                        raise wrap_exception(ORTModuleIOError, RuntimeError(msg))
    ...
    try:
        ...
        tensors_generated_by_ort = [
            wrapped_arg for tensor_flag, wrapped_arg in zip(tensor_type_flags, wrapped_args) if tensor_flag
        ]
        ...
        with torch.set_grad_enabled(is_training_mode):
            ...
            unwrapped_values = wrap_all_outputs(result, tensors_generated_by_ort, is_training_mode)
